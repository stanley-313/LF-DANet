...
def shearLF(data, label, s, angRes):
    disparity = [-4, -3 - 2, -1, 0, 1, 2, 3, 4]
    d = random.choice(disparity)
    pad = (angRes-1) * abs(d)
    uh, vw = data.shape
    h, w = uh // angRes, vw // angRes
    data = data.reshape(angRes, h, angRes, w)
    label = label.reshape(angRes, h*s, angRes, w*s)
    if random.random() < 0.5: # only horizontal
        data_pad = np.zeros((angRes, h, angRes, w+pad), dtype='single')
        label_pad = np.zeros((angRes, h*s, angRes, (w+pad)*s), dtype='single')
        for i in range(5):
            for j in range(5):
                if d >= 0:
                    data_pad[i,:,j,d*j:d*j+w], label_pad[i,:,j,d*j*s:(d*j+w)*s] = data[i,:,j,:], label[i,:,j,:]
                else:
                    data_pad[i,:,j,pad+d*j:pad+d*j+w], label_pad[i,:,j,(pad+d*j)*s:(pad+d*j+w)*s] = data[i,:,j,:], label[i,:,j,:]

        data_pad = data_pad[:,:,:,pad//2:pad//2+w].reshape((h*angRes, w*angRes))
        label_pad = label_pad[:,:,:,s*pad//2:(pad//2+w)*s].reshape((h*s*angRes, w*s*angRes))
    else: # only vertical
        data_pad = np.zeros((angRes, h+pad, angRes, w), dtype='single')
        label_pad = np.zeros((angRes, (h+pad)*s, angRes, w*s), dtype='single')
        for i in range(5):
            for j in range(5):
                if d >= 0:
                    data_pad[i,d*i:d*i+h,j,:], label_pad[i,d*i*s:(d*i+h)*s,j,:] = data[i,:,j,:], label[i,:,j,:]
                else:
                    data_pad[i,pad+d*i:pad+d*i+h,j,:], label_pad[i,(pad+d*i)*s:(pad+d*i+h)*s,j,:] = data[i,:,j,:], label[i,:,j,:]

        data_pad = data_pad[:,pad//2:pad//2+h,:,:].reshape((h*angRes, w*angRes))
        label_pad = label_pad[:,s*pad//2:(pad//2+h)*s,:,:].reshape((angRes*h*s, w*s*angRes))

    return data_pad, label_pad

def augmentation(data, label, s, angRes):
    ...
    # data, label = shearLF(data, label, s, angRes)
    return data, label
...
class TrainSetLoader(Dataset):
    def __init__(self, dataset_dir, cfg):
        ...
        self.file_list = []
        self.scale_factor = cfg.upscale_factor
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(file_name[0], 'r') as hf:
            ...
            data, label = augmentation(data, label, self.scale_factor, self.angRes)
